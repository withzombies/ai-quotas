...
- **Claude** `GET api.anthropic.com/api/oauth/usage`: requires `anthropic-beta: oauth-2025-04-20` AND `User-Agent: claude-code/<version>` — without that UA the endpoint returns persistent 429s. The edge may 403 non-curl TLS fingerprints → curl subprocess fallback. `utilization` is already 0–100. Token needs scopes `user:inference` + `user:profile`.
